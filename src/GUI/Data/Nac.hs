...
propagateRuleChanges (lhs,lhsgi) ((nac,nacgi),(nm,em)) = ((nacFinal,nacgi2), (nm'',em''))
  where
    -- 1 remove from NAC elements from the LHS that are not in the mapping (deleted elements)
    nodesToRemove = filter (`notElem` (nodeIds lhs)) (M.keys nm)
    edgesToRemove = filter (`notElem` (edgeIds lhs)) (M.keys em)
    edgesToRemove' = map edgeId $ filter (\e -> edgeId e `elem` M.elems em && not (haveCorrectEndings (nm,em) lhs e)) (edges nac)
    ((nac1,nacgi1),(nm',em')) = removeFromNAC ((nac,nacgi),(nm,em)) (nodesToRemove,edgesToRemove `union` edgesToRemove')
...
-- given the morphism between the LHS and the NAC, the LHS graph and a edge of the NAC,
-- check if the edge endings preserve the morphism
haveCorrectEndings :: MergeMapping -> Graph a b -> Edge b -> Bool
haveCorrectEndings (nm,em) lhs e = b
  where
    b = case (eIdInL, srcIdInL, tgtIdInL) of
